...
// --- SKELETON COMPONENTS ---
const SkeletonMap = () => (
  <div className="w-full h-full bg-slate-200 animate-pulse flex items-center justify-center flex-col gap-4 overflow-hidden relative z-0">
...
const RecordScreen = ({ onSaveActivity }) => {
  ...
  const [gpsStatus, setGpsStatus] = useState('idle'); // idle, initializing, ready, error
  ...
  // Logic Izin Lokasi yang Kuat & Tahan Blokir Browser
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        if (err.code === err.PERMISSION_DENIED) {
          setErrorMsg('Izin lokasi ditolak. Silakan ketuk ikon gembok di URL browser Anda, izinkan lokasi, lalu coba lagi.');
        } else if (err.code === err.POSITION_UNAVAILABLE) {
          setErrorMsg('Informasi lokasi tidak tersedia. Pastikan GPS perangkat menyala.');
        } else {
          setErrorMsg('Permintaan lokasi terlalu lama (timeout). Coba lagi di ruang terbuka.');
        }
      },
  ...
  // Coba minta otomatis saat pertama kali buka tab (Jika gagal, jangan panik, UI akan menampilkan tombol)
  useEffect(() => {
  ...
  useEffect(() => {
    if (isRecording && !isPaused && gpsStatus === 'ready') {
      watchId.current = navigator.geolocation.watchPosition(
        ...
        (err) => {
           ...
           // Tidak ubah status error jika sedang merekam agar layar tidak hilang mendadak, cukup diam
        },
  ...
  }, [isRecording, isPaused, gpsStatus]);
  ...
  return (
    <div className="w-full h-full relative bg-slate-900 overflow-hidden flex flex-col md:block">
      ...
      <div className="flex-1 md:absolute md:inset-0 relative z-0 bg-slate-100">
        ...
        {gpsStatus === 'error' ? (
          // LAYAR ERROR EKSPLISIT (Solusi Utama Vercel)
          <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center text-slate-600 bg-slate-50 z-10 relative">
            ...
            <h2 className="text-2xl font-black text-slate-800 mb-2">Akses Lokasi Diblokir</h2>
            <p className="text-sm md:text-base font-medium mb-8 max-w-sm text-slate-500">
              {errorMsg} Browser sering memblokir pop-up otomatis. Tekan tombol di bawah untuk memicu ulang permintaan.
            </p>
            <button 
            ...
            >
              <MapPin className="w-5 h-5" /> Aktifkan GPS Sekarang
            </button>
          </div>

        ) : gpsStatus === 'initializing' || gpsStatus === 'idle' ? (
          <SkeletonMap />
        ) : (
          <MapView coordinates={coordinates} isLive={true} className="w-full h-full" />
        )}

        {/* GPS Badge (Hanya tampil jika Ready atau sedang Rekam) */}
        {gpsStatus === 'ready' && (
      ...
      {/* DASHBOARD BAWAH */}
      <div className={`z-10 bg-white md:bg-white/80 md:backdrop-blur-xl md:absolute md:bottom-8 md:left-1/2 md:-translate-x-1/2 md:w-[90%] md:max-w-4xl md:rounded-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.08)] md:shadow-2xl rounded-t-3xl -mt-6 p-6 pb-28 md:pb-6 transition-transform duration-500 border border-white/20 ${gpsStatus === 'error' ? 'translate-y-full md:translate-y-0 opacity-0 md:opacity-50 pointer-events-none' : 'translate-y-0 opacity-100'}`}>
          ...
          <div className="flex justify-center md:justify-end gap-4 shrink-0 md:pl-8 md:border-l border-slate-200">
            {!isRecording ? (
              <button 
                ...
                className="w-20 h-20 md:w-24 md:h-24 bg-orange-500 text-white rounded-full flex items-center justify-center shadow-[0_8px_30px_rgba(252,76,2,0.4)] hover:bg-orange-600 transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed"
              >
